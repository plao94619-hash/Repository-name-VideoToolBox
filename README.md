...
- Universal-Media-Toolbox-Setup-1.9.0.exe：安装版
- Universal-Media-Toolbox-Portable-1.9.0.zip：免安装便携版
- SHA256SUMS.txt：校验值
...
此功能仅用于你拥有或获授权编辑的图片，并且只处理手动框选的可见区域。它不检测、不定位，也不承诺移除 C2PA 内容凭证、版权归属、平台溯源、隐写标记或其他不可见指纹；项目不提供规避这些标记的专用能力。
